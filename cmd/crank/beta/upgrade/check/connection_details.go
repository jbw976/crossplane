...
const (
	...
	remediationCompositeConnectionDetails = "Composite-resource connection details are removed in Crossplane v2. Recreate by explicitly composing a Kubernetes Secret from the desired connection detail values."
)
...
func (c *CompositeConnectionDetails) Run(ctx context.Context) ([]Finding, error) {
	...
	for i := range comps.Items {
		...
		findings = append(findings, Finding{
			...
			Message:     "Composition sets spec.writeConnectionSecretsToNamespace, which is removed in Crossplane v2.",
			Remediation: remediationCompositeConnectionDetails,
		})
	}
	...
	for _, t := range types {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "cannot list instances of %s defined by XRD %q", t.GVK.String(), t.XRDName)
		}
		for i := range instances {
			inst := instances[i]
			ref, found, err := unstructured.NestedMap(inst.Object, "spec", "writeConnectionSecretToRef")
			if err != nil {
				return nil, errors.Wrapf(err, "cannot read spec.writeConnectionSecretToRef of %s %q", inst.GetKind(), inst.GetName())
			}
			if !found || ref == nil {
				continue
			}
			findings = append(findings, Finding{
				...
				Message:     "Composite sets spec.writeConnectionSecretToRef, which is removed in Crossplane v2.",
				Remediation: remediationCompositeConnectionDetails,
			})
		}
	}
